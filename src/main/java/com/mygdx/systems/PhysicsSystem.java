...
public class PhysicsSystem extends IteratingSystem implements Telegraph {
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        if(movement.getPhysicsInfluence() > 0.0f) {
            ...
            float dampingFactor = (movement.getVel().len2()*movement.getVel().len2())/100000000.0f;
            //clamp
            if(dampingFactor > 1.0f) {
                dampingFactor = 1.0f;
            }

            float damp = (1.0f - dampingFactor) * movement.getPhysicsInfluence();
        ...
        transform.setAngle(transform.getAngle() + movement.getAngVel() * deltaTime);
        transform.setPos(transform.getPos().mulAdd(movement.getVel(), deltaTime));
    ...
    public boolean handleMessage(Telegram msg) {
        ...
        MovementComponent mc01 = data.getFirst().getComponent(MovementComponent.class);
        MovementComponent mc02 = data.getSecond().getComponent(MovementComponent.class);
        TransformComponent tc01 = data.getFirst().getComponent(TransformComponent.class);
        TransformComponent tc02 = data.getSecond().getComponent(TransformComponent.class);
    ...
    public void addedToEngine(Engine engine) {
        ...
        MessageManager.getInstance().addListener(this, CollisionPhysicsMsg.MSG_ID);
    }
