...
public class RenderingSystem extends IteratingSystem {
    ...
    private ComponentMapper<VisualComponent> vc = ComponentMapper.getFor(VisualComponent.class);
    ...
    ShapeRenderer shapeRenderer = new ShapeRenderer(); //Rendert Geometrie zur Laufzeit
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        if(renderTexture) {
            ...
            img.setPosition(transform.getPos().x + Gdx.graphics.getWidth() / 2 - img.getWidth() / 2,
                    transform.getPos().y + Gdx.graphics.getHeight() / 2 - img.getHeight() / 2);
        ...
        shapeRenderer.translate(Gdx.graphics.getWidth()/2, Gdx.graphics.getHeight()/2, 0.0f);

        //render circle collider
        if(renderColliderCircle) {
            shapeRenderer.setColor(collision.getColor());
            shapeRenderer.point(transform.getPos().x, transform.getPos().y, 0.0f);
            shapeRenderer.circle(transform.getPos().x, transform.getPos().y, collision.getRadius());
        }

        //render shape
        if(renderShape) {
            shapeRenderer.setColor(visual.getColor());
            shapeRenderer.translate(transform.getPos().x, transform.getPos().y, 0);
